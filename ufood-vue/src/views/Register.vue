<template>
  <div class="wrapper">
    <div style="margin: 200px auto; background-color: #fff; width: 350px; height: 400px; padding: 20px; border-radius: 10px; opacity: 0.9;">
      ...
      <el-form :model="user" :rules="rules" ref="userForm">
        ...
        <el-form-item prop="confirmPassword">
          <el-input placeholder="请再次输入密码" size="medium" style="margin: 5px 0" prefix-icon="el-icon-lock" show-password v-model="user.confirmPassword"></el-input>
        </el-form-item>
  ...
  methods: {
    login() {
      this.$refs['userForm'].validate((valid) => {
        ...
        if (valid) {  // 表单校验合法
          if (this.user.password != this.user.confirmPassword) {    // 校验密码是否一致
